    ...
    class Builder
      ...
      private def build_user_content(text : String, media : Array(Bus::MediaAttachment)?) : JSON::Any
        ...
        if media && !media.empty?
          # ⚡ Bolt Optimization: Write directly to an IO buffer using String.build and Enumerable#join
          # This avoids creating intermediate arrays and strings during context generation.
          # Expected impact: Reduced memory allocations and slightly faster context assembly.

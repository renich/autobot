    ...
    class Builder
      ...
      private def build_user_content(text : String, media : Array(Bus::MediaAttachment)?) : JSON::Any
        ...
        if media && !media.empty?
          # ⚡ Optimize: Use String.build to avoid intermediate Array and String allocations
          content = String.build do |io|
            io << content << "\n\nMedia:\n"
            media.join(io, "\n") do |attachment, item_io|
              item_io << "[" << attachment.type << ": " << (attachment.file_path || attachment.url) << "]"
            end
          end
        end
